...
export default function BookingPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-white mb-8">Book a Service</h1>

      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
        {/* ---- Service Summary ---- */}
        <div className="card">
          <h2 className="text-lg font-bold text-white mb-4">Service Summary</h2>
          ...
          <div className="flex items-center gap-3 mb-6 pb-6 border-b border-surface-border">
            ...
            <div>
              <p className="text-white font-medium">{provider.user?.name}</p>
              <p className="text-slate-400 text-sm">{provider.user?.phone}</p>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white">₹{service.price}/{service.priceType === 'fixed' ? 'fixed' : 'hr'}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">Duration</span>
              <span className="text-white">{form.duration} hour{form.duration > 1 ? 's' : ''}</span>
            </div>
        ...
        <div className="card">
          ...
          {reviews.length === 0 ? (
            <p className="text-slate-400 text-sm">No reviews yet for this provider.</p>
          ) : (
                ...
                <div key={rev._id} className="p-4 rounded-xl bg-surface border border-surface-border">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-white text-sm font-semibold">{rev.user?.name || 'User'}</p>
                  ...
                  <p className="text-slate-300 text-sm mt-2 font-medium">{rev.title}</p>
                  <p className="text-slate-400 text-sm mt-1">{rev.comment}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="card">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button type="button" onClick={() => setForm({ ...form, duration: Math.max(1, form.duration - 1) })}
                    className="w-10 h-10 rounded-lg bg-surface-card border border-surface-border text-white hover:border-primary-500 transition-all flex items-center justify-center text-xl">−</button>
                  <span className="text-white text-xl font-bold w-8 text-center">{form.duration}</span>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Special Instructions (optional)</label>
